...
def ensure_docker():
    ...
    if not os.path.exists(DETECT_DOCKER_SCRIPT):
        print(f"Error: Docker detection script not found at {DETECT_DOCKER_SCRIPT}")
        sys.exit(1)
    try:
        subprocess.run(
        ...
        print("Error: Not running inside Docker.")
        sys.exit(1)
